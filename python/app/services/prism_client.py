...
class PrismClient:
    ...
    async def offline_log(
        ...
        # Fire-and-forget: dispatch MongoDB logging to a background task so it doesn't block the caller
        try:
            asyncio.create_task(
            ...
            logger.warning("[PRISM] MongoDB telemetry background task dispatch failed: %s", e)
